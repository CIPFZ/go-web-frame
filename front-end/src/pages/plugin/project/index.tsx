...
const projectStatusMap: Record<ProjectStatus, { label: string; color: string }> = { planning: { label: '筹备中', color: 'gold' }, active: { label: '已发布', color: 'success' }, offlined: { label: '已归档', color: 'default' } };
const releaseStatusMap: Record<ReleaseStatus, { label: string; color: string }> = { draft: { label: '草稿', color: 'default' }, release_preparing: { label: '提交资料', color: 'gold' }, pending_review: { label: '审核中', color: 'processing' }, approved: { label: '待发布', color: 'cyan' }, rejected: { label: '已打回', color: 'error' }, released: { label: '已发布', color: 'success' }, offlined: { label: '已下架', color: 'volcano' } };
const requestTypeMap: Record<RequestType, string> = { initial: '首发', maintenance: '版本更新', offline: '下架申请' };
...
const PluginProjectPage: React.FC = () => {
  ...
  const loadProject = async () => {
    ...
    setLoading(false);
    if (!res || res.code !== 0) return message.error(res?.msg || '加载项目详情失败');
    const versions = [...(res.data?.versions || [])].sort((a, b) => getTime(b.createdAt) - getTime(a.createdAt));
    const detail = { ...res.data, versions };
    setProject(detail);
    const preferred = versions.find((item) => ['pending_review', 'approved', 'rejected', 'release_preparing', 'draft'].includes(item.status)) || versions[0];
  ...
  const stepItems = selectedVersion?.requestType === 'offline' ? [{ title: '下架申请' }, { title: '下架审核' }, { title: '执行下架' }] : [{ title: '提交资料' }, { title: '审核' }, { title: '发布' }];
  ...
  return (
    <PageContainer loading={loading} title={false} content="项目详情页承载项目基础信息、版本管理和版本流程。先选版本，再查看对应资料、审核与发布时间轴。">
      {project ? (
        <Row gutter={[16, 16]}>
          <Col xs={24} lg={8} xl={6}>
            <Space direction="vertical" size={16} style={{ width: '100%', position: 'sticky', top: 24 }}>
              <Card bordered={false} style={cardStyle} bodyStyle={{ padding: 24, textAlign: 'center' }}>
                <Button type="link" icon={<ArrowLeftOutlined />} style={{ paddingInline: 0, marginBottom: 16, float: 'left' }} onClick={() => history.push('/plugin/center')}>返回项目管理</Button>
                ...
                <Descriptions column={1} size="small" style={{ marginTop: 24, textAlign: 'left' }} labelStyle={{ width: 88 }}>
                  ...
                  <Descriptions.Item label="仓库地址"><Space size={6}><Typography.Text ellipsis style={{ maxWidth: 150 }}>{project.repositoryUrl || '-'}</Typography.Text>{project.repositoryUrl ? <Tooltip title="复制仓库地址"><Button type="text" size="small" icon={<CopyOutlined />} onClick={() => navigator.clipboard.writeText(project.repositoryUrl)} /></Tooltip> : null}</Space></Descriptions.Item>
                </Descriptions>
              ...
              <Card bordered={false} style={cardStyle}>
                ...
                <div style={{ background: '#f8f9fa', padding: 16, borderRadius: 8 }}>
                  ...
                  <Space align="center" size={16} wrap>
                    <Select value={selectedVersion?.ID} style={{ width: 320 }} options={(project.versions || []).map((item: any) => ({ label: `${item.version ? `v${item.version}` : requestTypeMap[item.requestType]} / ${releaseStatusMap[item.status]?.label || item.status}`, value: item.ID }))} onChange={(value) => { setSelectedVersionId(value); void hydrateReleaseDetail(value); }} />
                    {selectedVersion ? <Space wrap size={8}><Tag color="blue">{requestTypeMap[selectedVersion.requestType]}</Tag><Tag color={releaseStatusMap[selectedVersion.status]?.color}>{releaseStatusMap[selectedVersion.status]?.label}</Tag></Space> : <Tag>暂无版本</Tag>}
                  </Space>
                </div>
              </Card>
              {selectedVersion ? (
                <>
                  <Card bordered={false} style={cardStyle}><Typography.Text type="secondary">版本流程</Typography.Text><Steps current={currentStep} status={selectedVersion.status === 'rejected' ? 'error' : undefined} items={stepItems} style={{ marginTop: 16 }} /></Card>
                  <Card bordered={false} style={cardStyle}>
                    <Tabs items={[
                      { key: 'overview', label: '版本概览', children: <Descriptions column={2} size="small" labelStyle={{ width: 120 }}><Descriptions.Item label="版本号">v{selectedVersion.version || '-'}</Descriptions.Item><Descriptions.Item label="发布人">{selectedVersion.publisher || '-'}</Descriptions.Item><Descriptions.Item label="适配 HCI">{pickCompat(selectedVersion.versionConstraint, 'HCI') || selectedVersion.versionConstraint || '-'}</Descriptions.Item><Descriptions.Item label="适配 ACLI">{pickCompat(selectedVersion.versionConstraint, 'ACLI') || '-'}</Descriptions.Item><Descriptions.Item label="中文变更说明" span={2}>{selectedVersion.changelogZh || '-'}</Descriptions.Item><Descriptions.Item label="English Changelog" span={2}>{selectedVersion.changelogEn || '-'}</Descriptions.Item></Descriptions> },
                      { key: 'files', label: '文件资料', children: <Row gutter={[16, 16]}>{[{ title: '测试报告', url: selectedVersion.testReportUrl }, { title: 'x86 插件包', url: selectedVersion.packageX86Url }, { title: 'ARM 插件包', url: selectedVersion.packageArmUrl }].map((item) => <Col xs={24} md={8} key={item.title}><Card size="small" style={cardStyle} bodyStyle={{ minHeight: 150 }}><Space direction="vertical" size={10} style={{ width: '100%' }}><FileTextOutlined style={{ fontSize: 20, color: '#1677ff' }} /><Typography.Text strong>{item.title}</Typography.Text><Typography.Paragraph type="secondary" ellipsis={{ rows: 2 }} style={{ minHeight: 42 }}>{fileName(item.url) || '暂未上传'}</Typography.Paragraph><Button href={item.url} target="_blank" disabled={!item.url}>下载文件</Button></Space></Card></Col>)}</Row> },
                      { key: 'review', label: '审核与发布', children: <Descriptions column={2} size="small" labelStyle={{ width: 120 }}><Descriptions.Item label="提交人">{userMap[selectedVersion.createdBy] || `#${selectedVersion.createdBy}`}</Descriptions.Item><Descriptions.Item label="提交时间">{selectedVersion.createdAt || '-'}</Descriptions.Item><Descriptions.Item label="审核人">{selectedVersion.reviewerId ? userMap[selectedVersion.reviewerId] || `#${selectedVersion.reviewerId}` : '-'}</Descriptions.Item><Descriptions.Item label="审核状态">{releaseStatusMap[selectedVersion.status]?.label || '-'}</Descriptions.Item><Descriptions.Item label="审核意见" span={2}>{selectedVersion.reviewComment || '-'}</Descriptions.Item><Descriptions.Item label="AI 扫描结果摘要" span={2}>待后端补充 AI 扫描字段，当前先保留展示区域。</Descriptions.Item><Descriptions.Item label="发布管理员">{selectedVersion.publisherId ? userMap[selectedVersion.publisherId] || `#${selectedVersion.publisherId}` : '-'}</Descriptions.Item><Descriptions.Item label="发布时间">{selectedVersion.releasedAt || selectedVersion.offlinedAt || '-'}</Descriptions.Item><Descriptions.Item label="全面扫描结果" span={2}>当前发布动作前提供确认步骤，全面扫描结果摘要待后端字段补充。</Descriptions.Item></Descriptions> },
                      { key: 'timeline', label: '流程时间轴', children: selectedVersion.events?.length ? <Timeline items={selectedVersion.events.map((item: any) => ({ color: item.toStatus === 'rejected' ? 'red' : ['released', 'offlined'].includes(item.toStatus) ? 'green' : 'blue', children: <Space direction="vertical" size={0}><Typography.Text strong>{item.action} / {item.fromStatus || 'none'} → {item.toStatus}</Typography.Text><Typography.Text type="secondary">{item.createdAt} / {userMap[item.operatorId] || `#${item.operatorId}`}</Typography.Text>{item.comment ? <Typography.Paragraph style={{ marginBottom: 0 }}>{item.comment}</Typography.Paragraph> : null}</Space> }))} /> : <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无流程时间轴" /> },
                    ]} />
                    {bottomActions}
                  </Card>
                </>
              ) : <Card bordered={false} style={cardStyle}><Empty description="当前项目还没有版本" /></Card>}
            </Space>
          </Col>
        </Row>
      ...
      <Modal open={!!reviewModal} title={reviewModal?.action === 'approve' ? '审核通过' : '打回版本'} confirmLoading={actionLoading} onCancel={() => { setReviewModal(undefined); setReviewComment(''); }} onOk={async () => { if (reviewModal?.action === 'reject' && !reviewComment.trim()) return message.warning('打回时必须填写意见'); const ok = await handleReleaseAction(reviewModal?.action || 'approve', reviewComment.trim()); if (ok) { setReviewModal(undefined); setReviewComment(''); } }}>
        <Typography.Paragraph>{reviewModal?.action === 'approve' ? '可填写审核备注，便于提交人和发布管理员了解审核结论。' : '请填写打回意见，提交人会根据这里的意见补充资料。'}</Typography.Paragraph>
        <Input.TextArea rows={4} value={reviewComment} onChange={(e) => setReviewComment(e.target.value)} placeholder={reviewModal?.action === 'approve' ? '请输入审核备注（可选）' : '请输入打回意见（必填）'} />
      </Modal>
    </PageContainer>
  );
};
