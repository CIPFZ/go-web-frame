...
import { Button, Card, Col, Divider, Empty, Input, Radio, Row, Select, Space, Spin, Tag, Typography, message } from 'antd';

import { getPluginList } from '@/services/market';
import type { PublishedPluginItem } from '@/types';
...
const inferCategory = (plugin: PublishedPluginItem): CategoryKey => {
  const text = [plugin.nameZh, plugin.nameEn, plugin.descriptionZh, plugin.descriptionEn, plugin.code].join(' ').toLowerCase();
...
const formatReleaseDate = (value?: string) => (value ? value.replace('T', ' ').replace('Z', '').slice(0, 10) : '-');

const getName = (lang: MarketLocale, item: PublishedPluginItem) => (lang === 'zh' ? item.nameZh : item.nameEn || item.nameZh);

const getDescription = (lang: MarketLocale, item: PublishedPluginItem) =>
...
const PluginPublicListPage: React.FC = () => {
  ...
  const loadPlugins = async (nextKeyword = '') => {
    setLoading(true);
    try {
      const res = await getPluginList(nextKeyword || undefined);
      if (res.code !== 0) {
  ...
  const filteredItems = useMemo(() => {
    ...
    const result = items.filter((item) => {
      const searchText = [item.code, item.nameZh, item.nameEn, item.descriptionZh, item.descriptionEn].join(' ').toLowerCase();
            ...
            <Row align="middle" justify="space-between" gutter={[12, 12]}>
              <Col>
                <Typography.Text type="secondary" style={{ fontSize: 13 }}>
                  {copy.found} <span style={{ color: '#1677ff', fontWeight: 600 }}>{filteredItems.length}</span> {copy.matches}
                </Typography.Text>
              </Col>
              <Col>
