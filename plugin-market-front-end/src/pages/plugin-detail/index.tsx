...
import { AppstoreOutlined, ArrowLeftOutlined, DownloadOutlined, FileTextOutlined, InfoCircleOutlined } from '@ant-design/icons';
import { Breadcrumb, Button, Card, Descriptions, Empty, Radio, Row, Col, Select, Space, Spin, Table, Tabs, Tag, Typography, message } from 'antd';
import type { ColumnsType } from 'antd/es/table';

import { getPluginDetail } from '@/services/market';
import type { PublishedPluginDetail, PublishedVersion } from '@/types';
import { getLocale, setLocale, type MarketLocale } from '@/utils/locale';

type MarketVersion = PublishedVersion;
...
const PluginPublicDetailPage: React.FC = () => {
  ...
  const [lang, setLangState] = useState<MarketLocale>(() => getLocale());

  const copy = copyMap[lang];

  const updateLang = (value: MarketLocale) => {
    setLangState(value);
    setLocale(value);
  };
  ...
  useEffect(() => {
    ...
    const loadDetail = async () => {
      ...
      try {
        ...
        message.error(error instanceof Error ? error.message : copy.loadFailed);
      } finally {
        setLoading(false);
      }
    };
    ...
    ? [
        {
          ...
          children: (
            <div style={{ padding: '8px 0' }}>
              ...
              <Typography.Paragraph style={{ lineHeight: 1.8 }}>{getDescription(lang, detail)}</Typography.Paragraph>

              <Typography.Title level={5} style={{ marginTop: 24, marginBottom: 12 }}>
                {copy.capabilities}
              </Typography.Title>
              <Typography.Paragraph style={{ lineHeight: 1.8 }}>{getCapability(lang, detail)}</Typography.Paragraph>
            </div>
          ),
        },
        {
        ...
        },
        {
          ...
          children: (
            <div style={{ padding: '8px 0' }}>
              ...
              <Typography.Paragraph style={{ lineHeight: 1.8 }}>{getPerformance(lang, selectedVersion)}</Typography.Paragraph>
            </div>
          ),
        },
        {
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Breadcrumb
            items={[
              ...
              {
                title: <Typography.Text strong>{loading ? copy.loading : getName(lang, detail) || copy.detail}</Typography.Text>,
              },
            ]}
          />
          <Radio.Group size="small" value={lang} onChange={(event) => updateLang(event.target.value)} buttonStyle="solid">
            <Radio.Button value="zh">{copy.languageZh}</Radio.Button>
                ...
                <Col xs={24} lg={8}>
                  <div style={{ background: '#fafafa', padding: 20, borderRadius: 4, height: '100%' }}>
                    ...
                    >
                      ...
                      <Descriptions.Item label={copy.publisher}>{selectedVersion?.publisher || detail.plugin.ownerName || '-'}</Descriptions.Item>
                    </Descriptions>
                  </div>
                </Col>
              </Row>
