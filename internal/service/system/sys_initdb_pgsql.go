...
func (h PgsqlInitHandler) WriteConfig(ctx context.Context) error {
	...
	//}
	//global.G.Config.System.DbType = Pgsql
	//global.G.Config.Pgsql = c
	//global.G.Config.JWT.SigningKey = uuid.New().String()
	//cs := utils.StructToMap(global.G.Config)
	//for k, v := range cs {
	//	global.G.Viper.Set(k, v)
	//}
	//return global.G.Viper.WriteConfig()
	return nil
}
...
func (h PgsqlInitHandler) InitData(ctx context.Context, inits initSlice) error {
	...
	for i := 0; i < len(inits); i++ {
		if inits[i].DataInserted(next) {
			fmt.Printf(InitDataExist, Pgsql, inits[i].InitializerName())
			continue
		}
		if n, err := inits[i].InitializeData(next); err != nil {
			fmt.Printf(InitDataFailed, Pgsql, inits[i].InitializerName(), err)
			return err
		} else {
			next = n
			fmt.Printf(InitDataSuccess, Pgsql, inits[i].InitializerName())
		}
	}
	fmt.Printf(InitSuccess, Pgsql)
	return nil
}
